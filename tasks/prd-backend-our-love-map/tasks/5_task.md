...
Implementar o `map-service.ts` com todas as operações de banco de dados para mapas e localizações: criação, ativação, consulta por token, atualização de status e verificação de expiração. Este serviço é consumido pelas rotas e pelo `payment-service`. Todos os acessos ao banco são feitos via `MapModel` e `LocationModel` (mongoose).
...
- Ativar mapa após pagamento aprovado (definir slug, token, `expiresAt`, status `active`)
...
- Buscar status de pagamento por `mapId`
- Buscar mapa por `order_nsu` (necessário no webhook)
</requirements>

## Subtarefas

- [x] 5.1 Implementar `src/services/map-service.ts` com as seguintes funções:
  - `createMap(data: CreateMapData): Promise<MapDocument>`
    - Valida limite de localizações por plano antes de inserir
    - Cria documento `Map` com status `pending_payment`
    - Cria documentos `Location` em batch via `LocationModel.insertMany`
  - `activateMap(mapId: string): Promise<MapDocument>`
    - Gera slug via `slug.ts`
    - Gera token via `token.ts`
    - Calcula `expiresAt`: `basic` → `now + 7 dias`; `premium` → `undefined`
    - Atualiza status para `active` via `MapModel.findByIdAndUpdate`
  - `setPaymentFailed(mapId: string): Promise<void>`
  - `getMapByToken(token: string): Promise<MapDocument | null>`
    - Se `expiresAt` < `now` e status `active` → chamar `expireMap` antes de retornar
  - `getMapByOrderNsu(orderNsu: string): Promise<MapDocument | null>`
  - `getPaymentStatus(mapId: string): Promise<MapPaymentStatus>`
  - `updatePaymentData(mapId: string, data: PaymentData): Promise<void>`
    - Salva `checkoutUrl` via `MapModel.findByIdAndUpdate`
- [x] 5.2 Definir interfaces TypeScript: `CreateMapData`, `MapPaymentStatus`, `PaymentData`
- [x] 5.3 Escrever testes unitários para `map-service.ts`
...
Todos os erros de operação do mongoose devem ser propagados (não silenciar).

## Critérios de Sucesso

- `createMap` rejeita com 422 se número de localizações exceder o limite do plano
- `activateMap` persiste slug, token e `expiresAt` corretos para cada plano
...
- [x] `test/services/map-service.test.ts` (com mock dos models Mongoose):
  ...
  - `activateMap` → slug gerado corretamente, token com 5 chars, `expiresAt` = now+7d para `basic`
  - `activateMap` → `expiresAt` = undefined para `premium`
...
- `./backend/src/models/map-model.ts` (dependência)
- `./backend/src/models/location-model.ts` (dependência)
